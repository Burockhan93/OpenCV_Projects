...
void stitch_images(StitchingMethod method) {
	...
	Mat panorama;
	if (method == StitchingMethod::Custom) {

		vector<Mat> cumulativeH = estimate_cumulative_homographies(images);
		Mat panorama = warp_and_stitch(images, cumulativeH);
	}
	else if (method == StitchingMethod::Default) {
		// Use OpenCV's built-in stitching
		cv::Ptr<cv::Stitcher> stitcher = cv::Stitcher::create();
		cv::Stitcher::Status status = stitcher->stitch(images, panorama);
		if (status != cv::Stitcher::OK) {
			cerr << "Error during stitching: " << int(status) << endl;
			return;
		}
	}
	...
	imwrite("resources/panorama/panoDef.jpg", panorama);
	waitKey(0);
}
